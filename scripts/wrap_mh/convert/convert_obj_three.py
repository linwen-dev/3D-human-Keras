...
# for https://github.com/mrdoob/three.js/blob/master/src/loaders/JSONLoader.js
...
def parse_mtl(fname):
    ...
    for line in fileinput.input(fname):
        ...
        if len(chunks) > 0:
            ...
            if chunks[0] == "Ke" and len(chunks) == 4:
                materials[identifier]["colorEmissive"] = [float(chunks[1]), float(chunks[2]), float(chunks[3])]
...
def readFace(faces):
    """
    Format an array of faces

    This format has a type number which is a multiple flag.
    Then depending on the flag up to 21 fields
    """
    # Note: to pandas array to qc
    # import pandas as pd
    # faces = readFace(basedata['faces'])
    # for row in faces:
    #     for k in ['faceVertexUvs','verts']:
    #         for n,v in zip('ijkl',row[k]):
    #             row[k+'_'+n]=v
    #         del row[k]
    # df=pd.DataFrame.from_dict(faces)
    ...
    while i < fl:
        ...
        row['faceType'] = faceType = faces[i]
        i+=1

        if getBit(faceType, 0):
            vn = 4
        else:
            vn = 3
        row['verts'] = faces[i:i + vn]
        i += vn

        if getBit(faceType, 1):
            row['material'] = faces[i]
            i += 1
        if getBit(faceType, 2):
            row['hasFaceUvs'] = faces[i]
            i += 1
        if getBit(faceType, 3):
            row['faceVertexUvs'] = faces[i:i + vn]
            i += vn
        if getBit(faceType, 4):
            row['faceNormals'] = faces[i]
            i += 1
        if getBit(faceType, 5):
            row['faceUvs'] = faces[i:i + vn]
            i += vn
        if getBit(faceType, 6):
            row['faceColors'] = faces[i]
            i += 1
        if getBit(faceType, 7):
            row['faceVertexColors'] = faces[i:i+vn]
            i += vn
        data.append(row)
...
def generate_face(f, fc):
    """
    This format is parse by the threejs loader.

    It is an array where the first value is a bit flag describing which of the
    4-21 possible columns are to follow.
    """
    ...
    # try to include group here TEST
...
def generate_morph_targets(morphfiles, n_vertices, infile):
    ...
    morphTargets = [generate_morph_vertex(vname, vertices) for vname, vertices in morphVertexData]
    return morphTargets
...
def convert_ascii(infile, morphfiles, colorfiles, outfile):
    ...
    if not file_exists(infile):
        raise ValueError("Couldn't find [%s] from %s" % (infile, os.path.abspath('.')))
        return
    ...
    print "%d vertices, %d faces, %d materials, %d morph targets" % (len(vertices), len(faces), len(materials), len(morphTargets))
